...
def distance_travelled(starting_point, maze_path, direction):
    if direction == 'up':
        ...
        if len(np.where(possible_ends <start)[0]) ==0:
            #guard is leaving
            next_direction='done'
            return np.array([0,starting_point[1]]),next_direction
    ...
    if direction == 'down':
        ...
        if len(np.where(possible_ends >start)[0]) ==0:
            #guard is leaving
            next_direction='done'
            return np.array([len(maze_path),starting_point[1]]),next_direction
    ...
    if direction == 'right':
        ...
        if len(np.where(possible_ends >start)[0]) ==0:
            #guard is leaving
            next_direction='done'
            return np.array([starting_point[0],len(maze_path)]),next_direction
    ...
    if direction == 'left':
        ...
        if len(np.where(possible_ends <start)[0]) ==0:
            #guard is leaving
            next_direction='done'
            return np.array([starting_point[0],0]),next_direction
    ...
    return end_point,next_direction
...
while direction is not 'done':
    ...
    starting_point,direction = distance_travelled(starting_point, maze_path, direction)
